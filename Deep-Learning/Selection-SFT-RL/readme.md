...
¹估算方法：
完全正确 ≈ cor_mean − 近似正确；
近似正确 ≈ cor_mean − 2×完全正确（只差 1 给 +1）。

关键信息解读 ──────────────────────────────

1. 格式合规率
   SFT 带来的格式优势稳定存在（旧 +3 pt，新 +3 pt）。
2. 数值奖励
   • 纯 GRPO 两版 cor_mean 都保持 1.22 ≈ 30 % 完全命中。
   • 新奖励把“差 1”记 1 分，因而 cor_mean 分布被拉平：
   – 完全正确率相同，但多了 30 %–50 % 的“近似正确”样本。
3. 总 reward
   纯 GRPO 依旧最高；SFT+GRPO 因 cor 分拉低总 reward ≈1.9。
   （若把差 1 权重调成 1.5，可弥补这一差距。）
4. 梯度稀疏性
   投票 + 差 1 显著降低“全零奖励”批次：
   – frac_reward_zero 维持 0.30–0.40；早期训练更稠密。
5. 训练时间与显存
   新脚本把样本增至 3.5 k，gen=8，时长仍控制在 25 min，显存 ≈55 GB。

结论：

• 近似正确的提升 **不会直接提高 GSM8K official 准确率**，但
– 让训练更稳、更快收敛；
– 为二阶段精修、反思、或教学型场景打下基础；
– 在 Curriculum RL 里是把“完全错误→完全正确”拆分成两步的中间站。
• 如果你的最终目标就是 Kaggle 排名、必须 exact match，那可以在后半段训练把「差 1」奖励权重调低，或者再跑一轮 `diff==0 → +2, else 0` 收尾。