...
> ① 数据来源：CUDA C Programming Guide v12.4，Compute Capability 8.0。
...
1. **线程 ⇢ warp ⇢ 指令**
   32 条线程组成 1 个 warp；Warp Scheduler 发射的是“warp 的下一条指令”，所以指令粒度＝warp。
2. **4 个调度器 vs. 64 个 FP32 Core**
   · 调度器＝“交通灯”：挑选就绪 warp 并发射指令（≤4 warp / ≤8 条指令每周期）。
   · FP32 Core＝“车道”：真正做加减乘除，64 条并行。
3. **并发上限**
   受寄存器/共享内存配额限制，A100 每 SM **最多驻留 48 warp = 1 536 线程**，
   全卡 (108 SM) 上限约 **5 184 warp / 165 888 线程**。

 GPU warp ≠ CPU 进程，也不完全像 CPU 线程；它更像“一条 32 lane 的 SIMD 指令束”——硬件一次把 32 条 GPU 线程锁步执行。
• GPU 线程（CUDA thread）≈ CPU SIMD 向量中的 **1 个标量 lane**，远比 CPU 线程（由操作系统调度）轻量。

──────────────────────────
GPU / CPU 概念对照表（便于放正文，一眼看区别）

| 维度          | GPU 线程 (CUDA thread)                | GPU Warp                     | CPU 线程 (OS thread)       | CPU SIMD Vector           |
| ------------- | ------------------------------------- | ---------------------------- | -------------------------- | ------------------------- |
| 典型规模      | 32 条线程 = 1 warp（固定）            | 32 条线程                    | 通常 1 ~ 数十/核           | 4 / 8 / 16 / 32 lane      |
| 调度者        | GPU 硬件（SM 内 Warp Scheduler）      | GPU 硬件                     | 操作系统 + CPU 内核        | CPU 指令解码器            |
| 调度粒度      | 执行单元感知不到单个线程，只感知 warp | warp（锁步）                 | 线程（可抢占、上下文切换） | 指令（单条向量指令）      |
| 上下文重量    | 极轻：每线程少量寄存器                | 轻：32 条线程共享 1 PC       | 重：寄存器 + TLB + OS PCB  | 极轻：几个寄存器          |
| 独立指令流？  | 是（逻辑上）                          | 共用一条指令流               | 是                         | 一条向量指令包含所有 lane |
| 内存/地址空间 | 与同 block 内线程共享                 | 与线程相同                   | 进程级地址空间             | 与 CPU 线程相同           |
| 类比关系      | SIMD vector 中的 1 lane               | 一条 **32-lane** SIMD 指令束 | OS 级调度单元              | GPU warp 的“亲戚”         |

简单口语化类比

1. **GPU 线程** = SIMD 指令里的一个“标量槽位”。
2. **GPU warp** = 一条 32 lane 的 SIMD 指令，每次钟摆齐刷刷执行。
3. **CPU 线程** = OS 可独立调度的执行流（抢占、系统调用、私有栈）。
4. **GPU 指令** = 某个 warp 同时要执行的那一条机器指令（加法/乘法/访存等）。
