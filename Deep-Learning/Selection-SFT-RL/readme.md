...
2. 但 SFTTrainer 源码要求**数据集中必须存在 `completion` 这一列**（无论用不用）。为了省事就补了空 字符串占位，使得字段齐全、代码不报错。

3. 为什么不把 answer 放进 `completion`？
...
在你当前“小数据、快跑 1 epoch”的设置下，整串 CE 提供最稠密梯度；如果未来扩充 LIMO 到数万条并跑多 epoch，可以考虑方案 C，并在 RL 阶段继续用格式奖励守护模板，以获得更高数值准确率且不过拟合冗长 COT。
...
都放进 completion，只让交叉熵监督这段文本，**保留一个格式类奖励仍然是最稳妥的做法**。理由与操作要点如下。
...
1. 推理温度 ≠ 0
   • SFT 时用 teacher forcing，一字不差地学到了标签；
   • 真正生成时往往设 temperature / top-p ≠ 0，模型有概率漏写或多写标签。
   格式奖励在 RL 阶段提供“在线纠错”作用，防止跑偏。
2. 交叉熵权重割裂
   • completion 里 `<reasoning>` 占 90 % token，`<answer>` 区只有几 token；
   • 若把标签全交给 CE，梯度仍偏向长推理内容；
   • 格式奖励给 `<answer>` 标签一个 **额外明确信号**，告诉模型“漏标签就 0 分”。
3. 安全带＋兼容“零 SFT” 情形
   • 将来若别的实验直接跳过 SFT，那么仍靠 fmt_reward 把模板拉回来。
...
| 设计项           | 建议                                                         |
| ---------------- | ------------------------------------------------------------ |
| 正则             | 与现在相同即可：<br>`^<reasoning>[\\s\\S]*?</reasoning>\\s*<answer>[\\s\\S]*?</answer>$` |
| 权重（fmt 加分） | 可从 **1.0 → 0.5** 逐渐调低：<br>• 前 50 步权重 1.0 让格式快速对齐<br>• 后续降到 0.2-0.5，把主要梯度让给数值奖励 |
| 罚分策略         | 若想更硬，可把 reward 改成 +1 / **-1**（漏标签扣分），对格式错误产生惩罚梯度 |
| 检查多行         | 仍然只需查 `c[0]`（第一条回答）；如要最稳，也可 `all()` 八条都检查再取 min |
...
1. **SFT**
   prompt = 题干   completion = `<reasoning>…</answer>`
   → 交叉熵确保模型生成整段模板 & 合理 COT。
2. **GRPO**
   ‑ **cor_reward**：完全命中数字 +2；其余 0。
   ‑ **fmt_reward**：正则匹配成功 +1；否则 0。
   权重可 (2, 0.5) 或 (2, 1) 取决于格式重要度。
3. 评测脚本
   仍然先检查格式 → 再抽 `<answer>` 数字，保持一致。
