...
伴随拐点，开源框架越来越重：PyTorch-2、MMCV-2、MMEngine、CUDA-Kernel……
新手稍不注意就会掉进“版本地狱”。

VAD (Vectorized Scene Representation for Efficient Autonomous Driving) 是目前唯一还能在 **单机单卡**、只用 **公开免费数据集** 跑通 “相机 + LiDAR + 时序” 全链路规划的项目。
...
VAD 采用 **BEV级融合 + 时序 ConvGRU**，并在输出端用 **Vectorization Head** 把栅格/框转成折线向量，直接供规划使用。

VAD 的根本优势有“两层”：

1. **传感器融合层面** – 同时利用多摄像头（产生 BEV 图像特征）和 LiDAR （产生体素／点云特征），并在 BEV 平面用 Deformable Cross-Attention 做几何对齐，兼顾
   • 相机提供的颜色、纹理、车道线等语义
   • LiDAR 提供的厘米级距离与轮廓
   => 感知更鲁棒、远近都准。
2. **表示与规划层面** – 把“融合后的 BEV 栅格”进一步**向量化**：
   • 地图 → 车道折线、道路边界折线
   • 动态目标 → 多模态运动折线
   • 规划结果 → Ego 车未来折线
   并用 3 条实例级折线约束（防碰撞、防越界、方向一致）直接训练规划。
   => 数据量从数十 MB 栅格压缩到几十 KB 折线；推理速度提升 2–9 倍；输出天然可解释（每条线对应一个实体）。

所以 VAD 的核心不只是“把 BEV 和 LiDAR 合起来”，而是：
