...
4. 策略训练时所用的提示（policy training prompts）。
   ...
   • 高质量的偏好数据在指令遵从性（instruction following）和真值性（truthfulness）上带来高达 8% 的性能提升。
...
在现代大语言模型（LMs）的开发流程中，往往会额外添加一个阶段——从偏好反馈中学习（有时也称 RLHF，reinforcement learning from human feedback）——再将模型投入实际使用[45, 35, 4]。先前有大量研究表明，该阶段能够显著增强模型，包括使模型在指令执行、代码生成、数学解题、文本摘要等方面[22, 28, 54, 67]都获得大幅度改善。
...
如图 1（原文示意）所示，PPO 与 DPO 都基于偏好数据进行模型训练，但过程并不相同：
