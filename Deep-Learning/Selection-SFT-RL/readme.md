...
监督微调（Supervised Fine-Tuning，简称SFT）相当于你作为老师，自己先列出很多问题，再告诉模型标准的回答：
...
| 字段                               | 含义及计算                                          | 典型范围   |
| ---------------------------------- | --------------------------------------------------- | ---------- |
| completions/mean_length            | 8 条回答平均长度（token）                           | 80-110     |
| completions/clipped_ratio          | 因触达 `max_completion_length` 被截断的比例         | 0.1-0.4    |
| completions/mean_terminated_length | 去掉超长剪裁后平均有效长度                          | 75-95      |
| rewards/cor_reward/mean            | 数值奖励均值（0/1/2）；≈完全正确率×2 + 近似正确率×1 | 0-2        |
| rewards/fmt_reward/mean            | 格式奖励均值（0/1）；≈格式合规率                    | 0-1        |
| reward                             | 两项相加的 batch 平均值                             | 0-3        |
| reward_std                         | batch 内 reward 标准差                              | 0-1.5      |
| frac_reward_zero_std               | batch 内 reward=0 的样本比例                        | 0-1        |
| kl                                 | 当前策略与 base 模型平均 KL 散度                    | 0-0.3 常见 |
| clip_ratio/*                       | PPO/GRPO 内部 log-prob 剪裁比例（本例为 0）         | 0-0.2      |
| train_runtime / steps/sec          | 同 SFT，表示整条 GRPO 训练耗时与速度                |            |

──────────────────────────────────
快速判读技巧
──────────────────────────────────

1. 看 fmt_reward_mean 能否 >0.9，代表 90 % 输出格式正确。
2. 看 cor_reward_mean：
   − 若奖励 0/2：÷2 ≈ 完全正确率
   − 若奖励 0/1/2：估算
   完全正确率 ≈ cor_mean – 近似正确率
3. KL 最好 <0.3；跳升说明 β 过小或学习率过高。
4. loss 在 GRPO 中只是 `β·KL – reward`，上下波动正常，看 reward 曲线更直观。
5. grad_norm 连续高于阈值（>10）说明需调 `max_grad_norm` 或学习率。

按以上字段，您就可以快速比较 “纯 GRPO” 与 “SFT+GRPO”：
• 格式率：看 fmt_reward_mean 是否显著高；
• 完全正确率：看 cor_reward_mean 或 reward_mean；
• 稳定性：看 KL 是否更低、grad_norm 是否更平稳。
